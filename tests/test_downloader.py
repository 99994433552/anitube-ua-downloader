...
from aniloader.downloader import sanitize_filename, VideoDownloader
...
class TestVideoDownloader:
    ...
    def downloader(self):
        ...
        return VideoDownloader(use_aria2c=False)

    @pytest.fixture
    def anime_series(self):
    ...
    def test_generate_episode_filename_series(self, downloader, anime_series):
    ...
    def test_generate_episode_filename_series_season_2(self, downloader, anime_series):
    ...
    def test_generate_episode_filename_movie_with_year(self, downloader, anime_movie):
        """Test filename generation for movie with year."""
        episode = Episode(number=1, data_id="0_0", data_file="test.m3u8")
        filename = downloader.generate_episode_filename(anime_movie, episode)
        assert filename == "Test Movie (2024).mp4"

    def test_generate_episode_filename_movie_without_year(self, downloader):
        ...
        episode = Episode(number=1, data_id="0_0", data_file="test.m3u8")
        filename = downloader.generate_episode_filename(anime, episode)
        assert filename == "Test Movie.mp4"

    def test_generate_filename_with_invalid_characters(self, downloader, anime_series):
    ...
    def test_create_output_directory_series(self, downloader, anime_series, tmp_path):
    ...
    def test_create_output_directory_movie_with_year(
        self, downloader, anime_movie, tmp_path
    ):
        """Test output directory creation for movie with year."""
        output_dir = downloader.create_output_directory(anime_movie, str(tmp_path))
        expected_path = tmp_path / "Test Movie (2024)"
        assert output_dir == expected_path
        assert output_dir.exists()
        assert output_dir.is_dir()

    def test_create_output_directory_with_invalid_chars(self, downloader, tmp_path):
        ...
        output_dir = downloader.create_output_directory(anime, str(tmp_path))
